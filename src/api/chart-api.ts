...
export class ChartApi<HorzScaleItem> implements IChartApiBase<HorzScaleItem>, DataUpdatesConsumer<SeriesType, HorzScaleItem> {
	...
	public addTrendline(x1: number, y1: number, x2: number, y2: number, options?: any): string {
    const id = 'trendline_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    const data = {
        id: id,
    ...
    return this._chartWidget.model().addTrendline(data, options);
}

public removeTrendline(id: string): void {
    this._chartWidget.model().removeTrendline(id);
}

public updateTrendline(id: string, x1?: number, y1?: number, x2?: number, y2?: number, options?: any): void {
